...
The *SAA* uses a probabilistic algorithm (*pYIN*; Cannam et al., 2010) to transcribe audio of people singing and scores it in real time using various statistical modelling approaches, mainly grounded in latent variable modelling and item response theory approaches. For long note singing, we identified three key factors: pitch accuracy, pitch volatility, and changes in pitch stability. For singing melodies, we built a model that takes into account aspects of the melody's structure, such as tonality and length, to predict how well a person can recall and sing back the melody.
...
In the paper, we discuss the importance of assessing musical production, particularly singing, and its integration with melodic memory research. We highlight some key points:
...
* 		Researchers developed an online singing test for data collection and analyzed it.
* 		They created a significant explanatory model explaining a moderate proportion of variance in the data, supporting their hypothesis that melodic complexity features are relevant predictors.
...
* 		The test was related to musical training, suggesting that singing abilities may be improved by such training.
* 		The study demonstrated that the analysis pipeline produced consistent results despite "dirty musical data."


## Experiment 2: Validation of the SAA “one-shot” paradigm

* 		Experiment 2 aimed to create an adaptive singing test by adding new features and implementing a one-shot paradigm for cleaner data.
...
* 		Demographic features like musical training, age, and gender had relatively small effects on singing ability.
* 		Automated transcription of sung recall data was compared to human transcription, with promising accuracy.
General Conclusions:
...
* 		The paper also has implications for the automatic assessment of musical performance in education.
* 		The work is open-source and encourages contributions from others in the field.
