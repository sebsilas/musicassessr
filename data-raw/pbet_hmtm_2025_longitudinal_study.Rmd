...
# Mapping for time ordering and labeling
time_order <- c("after one day" = 1,
                "after one week" = 2,
                "after two weeks" = 3,
                "after three weeks" = 4,
                "after four weeks" = 5)
...
# Transform data
sessions_matrix_long2 <- sessions_matrix %>%
  pivot_longer(cols = starts_with("test_"), names_to = "test_time", values_to = "practice") %>%
  mutate(
    test_time = str_replace(test_time, "test_after_", ""),
    practice = factor(practice, levels = c("TRUE", "FALSE")),
    test_time = factor(test_time, levels = names(time_labels))
  ) %>%
  mutate(practiced_days = str_split(condition, "\\|")) %>%
  rowwise() %>%
  mutate(
    n_sessions = length(practiced_days),
    sort_key = sum(unlist(time_order[unlist(practiced_days)]), na.rm = TRUE)
  ) %>%
  ungroup() %>%
  mutate(
    condition = fct_reorder(condition, desc(n_sessions * 10 + sort_key)),
    n_sessions = paste(n_sessions, ifelse(n_sessions == 1, "session", "sessions")) # format label
  )

# Plot with group headers and relabeled x-axis
p <- ggplot(sessions_matrix_long2, aes(x = test_time, y = condition, fill = practice)) +
  geom_tile(color = "white") +
  scale_fill_manual(values = c("TRUE" = "steelblue", "FALSE" = "gray90")) +
  scale_x_discrete(labels = time_labels) +
  facet_grid(n_sessions ~ ., scales = "free_y", space = "free") +
  # facet_wrap(~ n_sessions, ncol = 1, scales = "free_y", strip.position = "top") +
  labs(x = "Test Time", y = "Practice Condition", fill = "Practice") +
...
ggsave("longitudinal_study_grid.png", plot = p, width = 8, height = 6, dpi = 300)
