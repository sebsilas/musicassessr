...
musicassessr_js <- function(app_name = "",
  ...
  c(
    ...
    system.file("www/js/metronome.js", package = "musicassessr"),
...
record_audio_setup <- function(asynchronous_api_mode, app_name) {
  ...
  DIRS_TO_CREATE <- c('www', 'www/audio') # These are only used by local/musicassessr_aws testing
...
check_port_windows <- function(port = 3000) {
  ...
  res <- res[5:length(res)] %>%
    ...
    purrr::map_dfr(function(x){
      local_addr <- x[[3]]
      remote_addr <- x[[4]]
      local_port <- x[[3]] %>% stringr::str_split(":") %>% purrr::pluck(1) %>% last()
      remote_port <- x[[4]] %>% stringr::str_split(":") %>% purrr::pluck(1) %>% last()
